...
(function () {
  var MIN_LINKS = 8;
  /* Версия видна в заголовке окна и в уведомлении. Заведена после того, как
     владелец не смог понять, какая закладка у него стоит: «не, всё равно
     выдаётся старая версия, я даже переустановил эту кнопку» (2026-09-14). */
  var VERSION = 'v4';
  ...
  /* Отправляем на '*', а не на конкретный домен: при несовпадении (www,
     превью-адрес Vercel, домен с другим регистром) браузер молча выбрасывает
     сообщение, и отличить это от «админка не ответила» невозможно. Дерево
  ...
  function sendToAdmin(onDone) {
    ...
    var payload = { source: 'redevelopment-menu-capture', host: host, pageUrl: location.href, tree: text };
    opener.postMessage(payload, '*');
    /* Повтор через секунду: вкладка админки могла быть в фоне и только
       просыпаться, а Supabase после паузы отвечает не мгновенно. */
    setTimeout(function () {
      if (!acked) {
        try {
          opener.postMessage(payload, '*');
        } catch (e) {
          /* вкладку успели закрыть — молчим, ниже сработает таймаут */
        }
      }
    }, 1200);
    /* 6 секунд: фоновая вкладка тормозится браузером, плюс холодный старт
       базы. Прежние 2.5 секунды давали ложное «не ответила» на успешной
       записи. */
    setTimeout(function () {
      if (!acked) {
        window.removeEventListener('message', onAck);
  ...
  var header =
    host + ' — разделов: ' + items.length + '  ·  ' + VERSION +
    (usedFallback ? '  (по адресам ссылок, навигацию найти не удалось)' : '');
  ...
  var sent = sendToAdmin(function (ok, count) {
    if (ok) {
      toast('Меню снято: ' + count + ' разделов  ·  ' + VERSION, true);
    } else {
      /* Причины две, и они лечатся по-разному, поэтому называем именно ту,
         которая случилась. */
