...
// В отрисованном тексте соседние элементы слипаются без пробела:
// «137-65-60contacts@idg-deco.ru» на decorazza.ru — это хвост телефона и
// начало почты из двух разных <span>. Отрезаем приклеенный номер: цифры с
// дефисами перед буквами. Ящик, который ВЕСЬ из цифр (89169287966@mail.ru
// на vsebloki.ru — обычное дело у мелких компаний), при этом не страдает.
function cleanEmail(value) {
return value.replace(/^\d[\d-]{4,}(?=[A-Za-z])/, '');
}
...
if (d.length === 12 && d.indexOf('375') === 0) return true;
if (d.length !== 11 || d.charAt(0) !== '7') return false;
// Код города/оператора в России начинается на 3, 4, 8 или 9 (495, 812, 800,
// 916). Первый живой прогон робота (2026-09-14) без этой проверки принёс с
// сайта завода «+7 (023) 392-33-89», «+7 (546) 533-08-45» и подобное — это
// ГОСТы и артикулы из описаний товаров, случайно похожие на номер.
if ('3489'.indexOf(d.charAt(1)) < 0) return false;
// Заглушка из маски поля ввода: «+7 (999) 999-99-99» на potolki.ru.
if (/^(\d)\1+$/.test(d.slice(1))) return false;
return true;
}
...
// «Поделиться страницей» — кнопка шаринга, а не контакт: на opalubka.pro
// в улов попало t.me/share/url?url=… (первый прогон робота, 2026-09-14).
var seg = abs.pathname.split('/').filter(Boolean)[0] || '';
if (/^(share|socks|proxy|joinchat|addstickers|iv)$/i.test(seg)) return null;
if (!seg) return null;
return { type: 'Telegram', value: abs.href };
}
