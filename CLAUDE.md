...
- **Перед завершением сессии — не оставлять готовую работу висеть на своей
  ветке, но и НЕ публиковать её самостоятельно.** 2026-09-10: за счёт того,
  что этот шаг регулярно пропускался, накопилось ~30 забытых веток с уже
  дописанным, но не долетевшим до `oodobu` кодом — разбор занял отдельную
  сессию (см. журнал за эту дату). С 2026-09-12 работает **очередь релиза**
  (см. следующий раздел): доведённая работа не вливается в `oodobu` сразу,
  а становится в очередь и ждёт, пока владелец скажет публиковать. Правило:
  если к концу сессии в её ветке лежат реальные, протестированные
  (`npx tsc -b`/`npm run build:app` чистые) изменения — запушить ветку,
  дописать запись в `docs/release-queue.md` и сказать об этом владельцу.
  Прямой пуш в `oodobu` технически проходит (проверено 2026-09-10 — прокси
  сессии блокирует только удаление чужих веток, не запись в них), но делать
  это без спроса нельзя (см. "Git Safety Protocol" харнесса) и в обычном
  порядке не нужно вовсе — публикует отдельная сессия, пачкой.


## Очередь релиза (с 2026-09-12)

Владелец ведёт несколько диалогов параллельно, доводит каждый до готовности,
архивирует — и публикует всё накопленное разом, **одним билдом**. Поэтому
публикация отделена от разработки.

**Что делает обычная (не публикующая) сессия, закончив работу:**

1. Пушит свою ветку (ту, что назначил харнесс), как и раньше.
2. Дописывает запись про себя в `docs/release-queue.md`, в начало раздела
   "Ждут публикации", по формату из самого файла: что меняется, чем
   проверено, применялись ли SQL-миграции, есть ли риск от задержки
   публикации, и какие конфликты ждать при сборке релиза.
3. Говорит владельцу: работа в очереди, публиковать когда скажет. Мерджить в
   `oodobu` и открывать PR самостоятельно НЕ нужно.

**Почему запись в очереди обязательна, а не "и так видно по веткам":** список
веток показывает ЧТО есть, но не показывает, закончено ли это, что уже
применено в живую базу и чем чревата задержка. Ровно этих данных не хватило,
когда очередь заводили (2026-09-12) — семь готовых веток висели непролитыми,
и по шести из них пришлось восстанавливать описание по коммит-месседжу.

**SQL-миграции — главная опасность отложенной публикации.** Миграции
применяются в живую базу сразу (см. раздел "SQL-миграции" выше), а код
доезжает до прода позже. Пока очередь не опубликована, база впереди кода,
поэтому:

- миграции держать **совместимыми со старым кодом**: добавить колонку/таблицу,
  дописать данные — можно; переименовать или удалить колонку, навесить
  `NOT NULL` на то, чего старый код не заполняет, ужесточить RLS так, что
  текущий прод перестанет видеть свои строки — **нельзя**, это сломает живой
  сайт до публикации;
- если ломающая миграция всё же нужна — не применять её в этой сессии, а
  записать в очередь как шаг, который выполняется ПОСЛЕ публикации кода;
- в записи очереди всегда указывать, что именно уже применено.

**Как публикуется очередь (по явной команде владельца):**

1. `git fetch origin`, прочитать `docs/release-queue.md`.
2. Отдельный `git worktree` от `oodobu`, ветка `claude/release-YYYY-MM-DD`.
3. Смержить туда все ветки из очереди по очереди, конфликты решать руками
   (журнал и сама очередь — сохранять обе записи; за остальным идти в поле
   "Конфликты при сборке" у записи).
4. `npm run build:app` — должен быть чистый. Если какая-то ветка ломает
   сборку — пересобрать релиз без неё, а её оставить в очереди с пометкой.
5. `create_pull_request` (GitHub MCP) из релизной ветки в `oodobu` — ОДИН PR
   на всю пачку, со списком, что внутри.
6. **Мержить сразу же, без отдельного подтверждения** (см. правило ниже) —
   это единственный момент, когда собирается прод-билд Vercel.
7. После мерджа: перенести записи в "Опубликовано" в `docs/release-queue.md`,
   сказать владельцу, какие ветки теперь можно удалить (удалить их самим
   обычно нельзя — нет прав на `git push --delete`).

**2026-09-13: команда "публикуй"/"опубликуй" уже включает мердж, второе
подтверждение не нужно.** Раньше публикующая сессия открывала PR и ждала
отдельного слова "мерджи" — владелец прямым текстом попросил это убрать
("если я сказал публиковать, значит заливаем на прод и мерджим по
умолчанию"): двойное подтверждение одного и того же действия — чистое
раздражение, а не защита от чего-то. Новое правило: любая явная команда
опубликовать/выкатить/влить очередь (в диалоге, где это по смыслу означает
"собери и вылей в прод") — открыть PR, сразу его смержить, дождаться
терминального состояния Vercel-деплоя и отчитаться, без промежуточного
"мерджить?". Не спрашивать заново то, что владелец уже один раз разрешил
на будущее. Если в конкретной ситуации есть реальный повод посомневаться
(сборка не собралась, конфликт непонятно как резолвить, миграция похожа на
ломающую) — не мержить и сказать почему, а не спрашивать "точно?" на ровном
месте.

Мерджить чужие ветки без явного разрешения владельца нельзя — харнесс это
и сам блокирует ("Modify Shared Resources", проверено 2026-09-12). Это
разрешение (публикующая сессия => мерджит) уже дано этим правилом — просить
его отдельно на каждый PR не нужно, только когда встретилась чужая
незапрошенная ветка вне обычного потока очереди.
