...
- unitPrice — цена за ОДНУ единицу ведомости, в единице позиции. Ставь число ТОЛЬКО когда: единица строки совпадает с единицей позиции (тогда это цена строки), ИЛИ известна тара строки (графа «тара» ниже либо объём в названии) и у позиции задан расход — цена ÷ объём тары × расход, ИЛИ известна тара и её содержимое измеряется в единице позиции — цена ÷ объём тары. Иначе null. Ничего не придумывай.
- НДС сам не добавляй и не вычитай: считай в той же базе, в какой дана цена строки. Включён ли НДС, определяется по условиям счёта, и пересчёт делает код, а не ты. Если из документа видно, что цены без НДС, — просто напиши это в note.
    ...
    lines
      .map(
        (l) =>
          ...
          // Тара строки, если её распознал счёт (шаг 7 плана закупок): «в
          // одной единице 9 л». Раньше модели оставалось выковыривать объём
          // из названия, а он там есть не всегда.
          (l.packQty && l.packUnit ? ` | тара: ${l.packQty} ${clean(l.packUnit, 12)} в одной ${clean(l.unit, 20) || 'единице'}` : '') +
