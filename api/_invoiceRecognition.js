...
// Порогов, ниже которых счёт НЕ записывается, здесь СОЗНАТЕЛЬНО нет.
// Владелец, 2026-09-16: «я не буду открывать письма руками и не увижу твоё
// предложение о распознавании счёта из текста — записывай по умолчанию».
// Распознали — записали; уверенность едет вместе с позициями
// (`recognitionConfidence` у PurchaseItem) и помечает ячейку «проверить» в
// сравнении цен, где на цифры и смотрят. Порог, оставляющий данные ждать
// человека в письме, которое никто не откроет, — это просто потерянные
// данные.
...
export function normalizeRecognized(parsed) {
  return {
    ...
    // Уверенность распознавания, 0-1 (шаг 10 плана закупок). Записи она не
    // мешает (см. комментарий про пороги выше) — она едет с позициями в
    // карточку и помечает ячейку «проверить» в сравнении цен. Ответ модели
    // без этого поля даёт null, и это НЕ то же самое, что ноль: null
    // означает «уверенность не спрашивали» (так отвечали до 2026-09-16).
    confidence:
