...
// новому, проверенному методу и одобрены" — ТОЛЬКО такие домены попадают
// в основную очередь вкладки "Верификация" (см. isReadyForVerification в
// SupplierVerificationTab.tsx) — вся остальная база (в т.ч. старые
// корректные снимки без этой отметки) временно вне очереди, пока не
// прогнана тем же методом и не помечена.
