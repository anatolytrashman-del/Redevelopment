...
// Email-адрес для переписки по конкретному предложению — plus-адресация на
// общем ящике снабжения (тот же приём, что был у покойной сущности
// Purchase, см. шаг 11b плана закупок): ответ
...
//
// 2026-09-16: формы отбрасываются СПИСКОМ СЛОВ, а не регуляркой с \b. Раньше
// здесь стояло /\b(ооо|оао|...)\b/g — и она не убирала НИ ОДНОЙ русской
// формы: граница слова в JS определена по ASCII, кириллица для неё не буква,
// и между пробелом и «о» границы нет (та же ловушка, что описана в
// CLAUDE.md). То есть «ООО Петрович» и «Петрович» кодом отождествлялись
// только на словах, а на деле считались разными компаниями — и подсказка
// «похоже, это дубль» на них не появлялась.
//
// Тот же список и то же правило — в api/_invoiceRouting.js
// (normalizeCompanyName): там по нему ищется поставщик из загруженного КП.
// Правится одно — правится и второе, иначе одна и та же пара названий будет
// «одной компанией» для подсказки о дубле и разными для маршрутизации счёта.
const SUPPLIER_LEGAL_FORMS = new Set([
  'ооо', 'оао', 'зао', 'пао', 'ао', 'ип', 'одо', 'уп', 'чуп', 'чтуп', 'тчуп',
  'чпуп', 'иооо', 'сооо', 'тоо', 'фоп', 'флп', 'iooo', 'llc', 'ltd', 'inc',
]);
...
export function normalizeSupplierName(name: string): string {
  return name
    ...
    .replace(/[.,]/g, ' ')
    .split(/\s+/)
    .filter((word) => word && !SUPPLIER_LEGAL_FORMS.has(word))
    .join(' ')
    .trim();
}
