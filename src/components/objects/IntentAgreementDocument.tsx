...
//
// dealMode переключает текст на формулировки об аренде (см. переключатель
// Покупка/Аренда на продающей странице) — стороны, предмет соглашения и
// ставка меняются, остальное (правовой статус, отсутствие платежей,
// характеристики помещения) общее для обоих режимов. Это ПРЕВЬЮ — реальный
// подписываемый документ живёт в гугл-шаблоне (INTENT_AGREEMENT_TEMPLATE_ID /
// RENT_AGREEMENT_TEMPLATE_ID и т.п., см. api/agreement-otp-verify.js), этот
// компонент только показывает клиенту, на что он примерно подписывается.
...
interface IntentAgreementDocumentProps {
  dealMode?: DealMode;
}

export function IntentAgreementDocument({ dealMode = 'sale' }: IntentAgreementDocumentProps) {
  const isRent = dealMode === 'rent';
  const partyB = isRent ? 'Арендатор' : 'Покупатель';
  const partyA = isRent ? 'Арендодатель' : 'Продавец';
    ...
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-6 rounded-card border border-border bg-surface p-6 text-ink sm:p-10">
      ...
      <div className="flex flex-col gap-3 text-sm leading-relaxed text-ink-muted">
        <p>
          ...
          <strong className="text-ink">{partyA}</strong>),
        </p>
        ...
        <p>
          ...
          <Blank width="8rem" /> (далее — <strong className="text-ink">{partyB}</strong>),
        </p>
      ...
      <Section number="1" title="Предмет и цель">
        <p>
          1.1. Стороны выражают взаимную заинтересованность в{' '}
          {isRent ? 'заключении в будущем договора аренды' : 'совершении в будущем сделки купли-продажи'} изолированного
          помещения (кабинета).
        </p>
      ...
      <Section number="2" title={isRent ? 'Предварительные условия аренды' : 'Предварительные условия сделки'}>
        {isRent ? (
          <p>
            2.1. Ориентировочная ежемесячная арендная плата за Помещение рассчитывается исходя из ставки 20 долларов США за
            1 кв. метр площади Помещения в месяц. Итоговый размер арендной платы определяется умножением указанной ставки
            на фактическую площадь Помещения (согласно данным ведомости технических характеристик / технического паспорта)
            и оплачивается в белорусских рублях по курсу Национального банка Республики Беларусь на день оплаты.
          </p>
        ) : (
          <p>
        ...
        {isRent ? (
          <p>
            2.2. Арендодатель намеревается завершить оформление прав на здание и обеспечить готовность Помещения (включая
            необходимые ремонтные и отделочные работы) к передаче в аренду.
          </p>
        ) : (
          <p>
        ...
        {isRent ? (
          <p>
            2.3. После завершения Арендодателем (или подконтрольным ему юридическим лицом) оформления прав на здание и
            готовности Помещения к передаче Стороны обязуются обсудить возможность заключения Договора аренды.
          </p>
        ) : (
          <p>
      ...
      <Section number="3" title="Правовой статус и отсутствие обязательств">
        ...
        <p>
          3.2. Ни одна из Сторон не берет на себя обязательств по обязательному заключению{' '}
          {isRent ? 'договора аренды' : 'договора купли-продажи'} в будущем.
        </p>
      ...
      <Section number="4" title="Подписи сторон">
        <div className="grid gap-4 sm:grid-cols-2">
          <SignatureBlock role={partyA} name={<Blurred>Трэшмен Анатолий Владимирович</Blurred>} />
          <SignatureBlock role={partyB} name={<Blank width="8rem" />} />
        </div>
      </Section>
    </div>
  );
