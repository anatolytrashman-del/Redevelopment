...
import { markWorkContractorEmailsRead, sendWorkContractorEmail } from '../../lib/workContractorEmailsApi';
...
// Вкладка "Подрядчики" страницы "Закупки" (владелец, 2026-09-14: "я хочу
// заложить основу для вкладки Подрядчики... мне на старте нужно всего два
// поля — ссылка на страницу на Авито и email подрядчика. С подрядчиками
// должна быть возможность общаться по email. Пока делаем только
// индивидуальные рассылки. Никаких автоматических файлов к письму не
// прикрепляется, я буду вручную писать текст и прикреплять все").
//
...
//
// Данные разделены с родителем: список подрядчиков живёт здесь, письма —
// на странице (Suppliers.tsx), потому что счётчик непрочитанных нужен
// бейджику вкладки ещё до того, как во вкладку зайдут, а содержимое
// неактивной вкладки не смонтировано.
interface WorkContractorsTabProps {
  emails: WorkContractorEmail[];
  onEmailsChange: (update: (prev: WorkContractorEmail[]) => WorkContractorEmail[]) => void;
}
...
export function WorkContractorsTab({ emails, onEmailsChange }: WorkContractorsTabProps) {
  ...
  }, [selectedId, emails, onEmailsChange]);
  ...
  async function handleDelete(c: WorkContractor) {
    ...
    try {
      ...
      onEmailsChange((prev) => prev.filter((e) => e.contractorId !== c.id));
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(260px,340px)_1fr]">
          ...
          {selected ? (
            <ContractorPanel
              ...
              onEmailSent={(email) => onEmailsChange((prev) => [...prev, email])}
            />
          ) : (
