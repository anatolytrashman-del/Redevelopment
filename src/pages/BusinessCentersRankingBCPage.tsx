...
const PAGE_URL = 'https://redevelopment.pro/minsk/bcminsk/rating/b-c';
...
export function BusinessCentersRankingBCPage() {
  ...
  const rankingB = useMemo(() => buildRankingForClasses(centers ?? [], ['B']), [centers]);
  const rankingC = useMemo(() => buildRankingForClasses(centers ?? [], ['C']), [centers]);
  const ranking = useMemo(() => [...rankingB, ...rankingC], [rankingB, rankingC]);
  const excluded = useMemo(() => buildExcludedForClasses(centers ?? [], ['B', 'C']), [centers]);
  ...
  const faqItems = useMemo(() => {
    ...
    items.push({
      ...
      answer: `Классы B и C показаны отдельно, нумерация в каждом списке начинается с первого места; только сданные здания в черте Минска, рейтинг Яндекс.Карт от ${RATING_THRESHOLD_LABEL} из 5 и не менее ${MIN_RATING_COUNT} оценок здания. Сортируем по рейтингу, при равенстве — по числу оценок, от большего к меньшему. Показываем все прошедшие порог здания без ограничения топ-N. Оценка относится к зданию целиком, а не только к офисам.`,
    });
    items.push({
      question: 'Почему классы B и C на одной странице?',
      answer:
        'Это более бюджетный сегмент: мы объединили его ради охвата, а не потому, что B и C — одно и то же. У классов разные характеристики; списки и места разделены, чтобы сравнивать здания внутри своего класса.',
    });
    for (const [businessClass, list] of [
      ['B', rankingB],
      ['C', rankingC],
    ] as const) {
      const total = centers.filter(
        (c) => c.businessClass === businessClass && c.status !== 'under_construction' && !isOutsideMinsk(c),
      ).length;
      const omissions = excluded.filter((e) => e.center.businessClass === businessClass);
      items.push({
        question: `Сколько зданий класса ${businessClass} попало в рейтинг?`,
        answer: `${list.length} из ${total} сданных зданий класса ${businessClass} в черте Минска.${summarizeExclusions(omissions)}`,
      });
      const leader = list[0];
      if (leader)
        items.push({
          question: `Какой бизнес-центр лидирует в классе ${businessClass}?`,
          answer: `«${shortName(leader.center)}» — ${leader.ratingLabel} из 5, ${ratingsCount(leader.ratingCount)}.`,
        });
    }
    if (ranking.length > 0) {
    ...
    return items;
  }, [centers, displayed, offerIndex, ranking, excluded, rankingB, rankingC]);
  ...
  return (
    <div className="min-h-svh bg-bg">
      ...
      <main data-menu-align className="mx-auto flex max-w-3xl flex-col gap-6 px-4 py-10 sm:px-8">
        ...
        <div className={cn('flex flex-col gap-3 p-6 sm:p-8', glassCardClass)} style={glassCardShadow}>
          ...
          <div className="text-xs text-ink-muted">
            ...
            <ol className="mt-2 flex list-decimal flex-col gap-1.5 pl-4 text-ink">
              <li>
                Классы B и C, каждый в отдельном списке (
              ...
              <li>Все прошедшие порог, по рейтингу; при равенстве — по числу оценок</li>
            </ol>
          </div>
        </div>
        ...
        {[
          { businessClass: 'B', items: rankingB },
          { businessClass: 'C', items: rankingC },
        ]
          .filter(({ items }) => items.length > 0)
          .map(({ businessClass, items }) => (
            <section key={businessClass} className="flex flex-col gap-3">
              <h2 className="text-lg font-bold text-ink">Класс {businessClass}</h2>
              {items.map((r, i) => {
