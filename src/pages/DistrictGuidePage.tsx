...
const trafficHighlights: { icon: LucideIcon; label: string; text: string }[] = [
  {
    icon: Building2,
    label: 'Жители района.',
  ...
  {
    icon: ShoppingBag,
    label: 'Запуск ТЦ и финцентра.',
    text: 'Avia Mall уже открыт, но ещё заселяется арендаторами, а Международный финансовый центр пока строится. Покупка или аренда помещений на этом этапе позволяет занять ликвидную точку до ввода этих объектов и роста ставок.',
  },
];
      ...
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-8">
        ...
        <div id="traffic" className={cn('flex scroll-mt-6 flex-col gap-3 p-6', glassCardClass)} style={glassCardShadow}>
          ...
          <ul className="flex flex-col gap-3">
            {trafficHighlights.map(({ icon: Icon, label, text }) => (
              <li key={label} className="flex items-start gap-2.5 text-sm text-ink-muted">
