    ...
    <>
      ...
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-8">
        ...
        {/* Раздел заполнен фактурой (владелец, 2026-08-26: "попроси Gemini
            ...
            ключ) + коммунальный кейс (Onliner, "Зеркало", середина 2026) —
            те же источники и факты, что и в первой версии абзацами, просто
            владелец увидел вживую и попросил короче: "херня, мне нужен
            короткий блок, давай снипеты про плюсы и минусы" — сплошной
            текст на всю карточку не читался, ужат в две колонки с
            короткими пунктами (по 3 в каждой) + одна строка юридической
            базы мелким текстом внизу. Указ №456 от 22.09.2014 — ТОТ ЖЕ
            указ, которым выделили землю под Минск Мир целиком (см. блок
            "Застройщик района" выше, та же дата), дополнен указом №370 от
            20.10.2020. Плюсы/минусы — не подгонка ради красивой таблицы,
            оба списка ровно по 3 пункта совпадают с тем, что реально
            обсуждалось в источниках (юридическая уникальность формата,
            гостиничная инфраструктура, ценность для инвестора — против
            нежилого статуса, отсутствия жилищных льгот и завышенной
            коммуналки). Лендинга под раздел по-прежнему нет — ссылку
            никуда не даю, только якорь с карточки в сетке выше. */}
            ...
            <div className="flex flex-col gap-2 rounded-control border border-white bg-white/60 p-4 sm:border-white/50 sm:bg-white/40">
              ...
              <ul className="flex flex-col gap-1.5">
                {[
                  ...
                  'Гостиничный сервис: ресепшен, охрана, консьерж, отделка под ключ',
                ].map((text) => (
            ...
            <div className="flex flex-col gap-1 rounded-control border border-dashed border-border p-4">
              ...
              <ul className="flex flex-col gap-1.5">
                {[
                  ...
                  'Коммуналка по тарифу для юрлиц — заметно дороже квартиры (были жалобы на 300–400 руб/мес)',
                ].map((text) => (
                  <li key={text} className="flex items-start gap-2 text-sm text-ink-muted">
