...
// Видимый бейдж свежести в hero считается из DATE_MODIFIED, а не хардкодится
// отдельной строкой (аудит 2026-08-26: текст «Обновлено: август 2026» жил
// сам по себе и гарантированно разошёлся бы с dateModified в Article JSON-LD
// при следующем обновлении — а расхождение видимой даты с разметкой хуже,
// чем её отсутствие). Обновлять теперь нужно только саму DATE_MODIFIED.
const UPDATED_BADGE_LABEL = (() => {
  const [year, month] = DATE_MODIFIED.split('-');
  return `Обновлено: ${MONTH_NAMES[Number(month) - 1]} ${year}`;
})();
