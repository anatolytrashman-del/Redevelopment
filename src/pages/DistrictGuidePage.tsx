...
// общему числу мест.
// 2026-08-25: ещё 2 точки убраны — 2 попали в выдачу из-за расширения
// поиска за пределы района (см. "Больше результатов рядом" в
// districtPlaces.ts) и 1 (Центр доктора Бубновского, ул. Короткевича) —
// за пределами района. Было 10.
const sportTotal = 7;
const sportBreakdown: { label: string; count: number }[] = [
  { label: 'Тренажёрные залы / фитнес-клубы', count: 5 },
  { label: 'Йога / пилатес / стретчинг', count: 3 },
];
...
const districtFaq: FaqItem[] = [
  ...
  },
  {
    ...
    answer:
      'В районе уже работает около 72 заведений общепита — плотное покрытие с явным перекосом в сторону кафе и кофеен (порядка 60 точек), что логично для молодой платёжеспособной аудитории района. Также представлены рестораны, бары, фастфуд, пекарни, пиццерии и кондитерские. Из локальных сетей заметна «Варка» (Varka) — 5 точек. Это одна из самых насыщенных ниш района, но высокая плотность здесь — одновременно и подтверждение устойчивого спроса на общепит у местных жителей.',
  },
  {
    question: 'Какая конкуренция в бьюти-сфере — салоны красоты, ногтевые студии, барбершопы?',
    answer:
      'Бьюти-сфера — один из самых насыщенных сегментов района: 74 салона и студии красоты. Лидируют по числу точек ногтевые студии и парикмахерские, также широко представлены стилисты, косметология, услуги по бровям и ресницам и барбершопы. Высокая плотность отражает и высокий спрос молодой аудитории района на бьюти-услуги в шаговой доступности, и заметную конкуренцию за клиента внутри самой ниши.',
  },
  {
  ...
  },
  {
    ...
    answer:
      'В районе работают 39 продуктовых точек — от крупных сетей до независимых лавок и специализированных магазинов. Заметнее всего представлена «Копеечка» (6 точек), также есть «Соседи»/«Соседи Экспресс», «Санта», «Евроопт Market», «Знічка», «Остров» и 8 специализированных точек (мясные, овощные, фермерские лавки). Это средний уровень плотности — категория не переполнена, как бьюти или общепит, но и без явного дефицита.',
  },
  {
    question: 'Есть ли в районе пункты выдачи заказов Wildberries и Ozon?',
    answer:
      'Да, в районе работают 20 пунктов выдачи Wildberries и 11 пунктов Ozon — суммарно 31 точка. Такая плотность отражает сильный спрос жителей района на покупки через маркетплейсы и объясняет, почему для этого формата, в отличие от многих других ниш, одной точки хватает на несколько соседних домов.',
  },
  {
  ...
  },
  {
    ...
    answer:
      'В районе работают 22 точки этого профиля — 15 вейп-шопов и 7 табачных магазинов.',
  },
  {
    question: 'Развита ли спортивная и фитнес-инфраструктура в районе?',
    answer:
      'В районе уже работает 7 залов и студий: 5 классических тренажёрных залов и фитнес-клубов полного цикла и 3 студии с акцентом на растяжку и осознанность — йога, пилатес, стретчинг. Это самая малочисленная из отслеживаемых категорий бизнеса района, что можно читать и как признак недонасыщенной ниши — спортивная инфраструктура уже востребована жителями, а конкуренция за помещение под неё будет только расти по мере роста населения.',
  },
  {
    ...
    <>
      ...
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-8">
        ...
        {/* Раздел заполнен фактурой (владелец, 2026-08-26: "попроси Gemini
            ...
            Источник юридической части — предпринимательский портал prometr.by
            (заголовок статьи прямо называет Указ №456 и №370) + официальная
            страница bir.by "Многофункциональные бизнес-апартаменты"
            (подтверждает начинку — гостиничные стандарты, отделка под
            ключ). Указ №456 от 22.09.2014 — ТОТ ЖЕ указ, которым выделили
            землю под Минск Мир целиком (см. блок "Застройщик района" выше,
            там та же дата) — установил особый режим для этого конкретного
            формата помещений, дополнен указом №370 от 20.10.2020.
            Коммунальный кейс (300–400 руб/мес, тариф 147% против 25–29% у
            обычной квартиры) — резонансная история середины 2026 года
            (Onliner, "Зеркало") — специально не замалчиваю: раздел не
            рекламный, важно дать честную картину формата, включая
            финансовый нюанс, который реально всплывал у покупателей. Раздел
            больше не заглушка, но лендинга под него по-прежнему нет —
            ссылку никуда не даю, только якорь с карточки в сетке выше. */}
        <div id="business-apartments" className={cn('flex scroll-mt-6 flex-col gap-3 p-6', glassCardClass)} style={glassCardShadow}>
          ...
          <p className="text-sm text-ink-muted">
            Юридически бизнес-апартаменты — нежилые помещения: их особый правовой режим установлен тем же указом
            Президента № 456 от 22 сентября 2014 года, которым в целом выделили землю под Минск Мир (дополнен указом
            № 370 от 20 октября 2020 года). Несмотря на нежилой статус, собственник и его близкие родственники —
            супруг(а), дети, родители — вправе зарегистрироваться в апартаментах по месту жительства, а по этому же
            адресу можно зарегистрировать бизнес — юридическое лицо любой формы собственности. Такого сочетания нет
            ни у обычной квартиры (там нельзя завести юрлицо), ни у обычного коммерческого помещения (там нельзя
            прописаться).
          </p>
          <p className="text-sm text-ink-muted">
            Застройщик позиционирует формат по гостиничным стандартам — ресепшен, охрана, консьерж-сервис, отделка
            под ключ, доступ к фитнес- и бассейн-инфраструктуре комплекса — что делает его удобным и для
            собственного проживания, и как инвестиционный актив под сдачу в аренду. Подходит в первую очередь
            предпринимателям и инвесторам, которым нужен один адрес для жизни и бизнеса или готовый формат под
            сдачу, — не семьям, ориентированным на типичные жилищные льготы.
          </p>
          <p className="text-sm text-ink-muted">
            Важный финансовый нюанс: коммунальные платежи по бизнес-апартаментам начисляются по тарифам для
            юридических лиц, а не по жилым (например, тариф на отопление для юрлиц — около 147% от экономически
            обоснованной стоимости против 25–29% для обычной квартиры). По сообщениям СМИ, в 2026 году часть
            собственников получила счета за коммунальные услуги в 300–400 рублей в месяц — заметно больше, чем
            изначально называл застройщик (разница в 20–30% от обычной квартиры).
          </p>
        </div>
