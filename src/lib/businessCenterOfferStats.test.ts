...
describe('buildDealStats', () => {
  it('сортирует лоты по цене и считает бюджет лота целиком', () => {
  ...
  it('ставит помещения по возрастанию цены, а не площади', () => {
    // Живой «Силуэт»: метр там гуляет всемеро при почти одинаковой
    // площади, поэтому сортировка по площади давала цены вразнобой.
    const totals = buildDealStats(SILUET, 'sale')!.lots.map((o) => Math.round(o.size * o.pricePerSqm));
    expect(totals).toEqual([...totals].sort((a, b) => a - b));
    expect(totals.slice(0, 3)).toEqual([4624, 4900, 5945]);
    // Порядок по площади при этом уже не соблюдается — и не должен.
    expect(buildDealStats(SILUET, 'sale')!.lots[0].size).toBe(6.1);
  });
