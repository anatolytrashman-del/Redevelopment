...
export const NEARBY_CATEGORY_LABELS: Record<NearbyPlaceCategory, string> = {
  ...
  bank: 'Банки',
  atm: 'Банкоматы',
  // Кафе и рестораны объединены в одну категорию (владелец, 2026-09-21):
  // на карте нет смысла разводить «Кухмистр» (кафе) и «Чарли» (ресторан)
  // по разным строкам легенды, когда обед ищут по обоим сразу. Кофейни —
  // наоборот, выделены из «кафе» отдельной категорией по тому же решению
  // («давай соберем кофейни отдельно»): рубрика «Кофейня» у Яндекса своя,
  // не совпадает с общим «Кафе».
  coffee: 'Кофейни',
